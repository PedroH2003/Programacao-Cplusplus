...
pair<int,int> compara(pair<int,int> a, pair<int,int> b){
...
pair<int,int> build(int p, int l, int r){
    ...
    int m = (l + r) / 2;
    return seg[p] = compara(build(p*2, l, m), build(p*2+1, m+1, r));
}
...
pair<int,int> update(int i, int x, int p, int l, int r){
    ...
    return seg[p] = compara(update(i, x, p*2, l, m), update(i, x, p*2+1, m+1, r));
}
...
pair<int,int> query(int a, int b, int p, int l, int r){
    ...
    return compara(query(a, b, p*2, l, m), query(a, b, p*2+1, m+1, r));
}
