...
/*Essa função é uma implementação do Algoritmo de Euclides para encontrar o máximo divisor comum (MDC) entre 
dois números inteiros a e b. O MDC é o maior número que divide exatamente a e b.*/

int mdc(int a, int b)
{
    if(b == 0)
    {
        return a; // se b for zero então a será o maximo divisor comum
    }

    return mdc(b, a % b); /* caso b não seja 0 iremos chamar novamente a função só que agora b seria a % b, 
    e iremos fazer isso até encontrarmos um b = 0*/
}